...
def compute_combined_score(
    ...
    """
    Compute a combined relevance score from multiple retrieval signals.

    This is the external version of the scoring formula used internally
    by UnifiedRetriever. Exposed here so consumers can reason about
    scores without importing the full retrieval pipeline.

    Default weights match UnifiedWeights defaults:
        vector: 0.45, tag: 0.15, graph: 0.15,
        co_retrieval: 0.10, recency: 0.10, confidence: 0.05

    Args:
        vector_score: Cosine similarity distance converted to [0, 1].
        tag_score: Fraction of query tags found in memory tags.
        graph_score: BFS graph traversal weight with depth decay.
        co_retrieval_score: NPMI association with top search results.
        recency_score: Exponential decay from last access.
        confidence: Memory's stated confidence field.
    ...
    total = 0.0
    total += weights.get("vector", 0) * vector_score
    total += weights.get("tag", 0) * tag_score
    total += weights.get("graph", 0) * graph_score
    total += weights.get("co_retrieval", 0) * co_retrieval_score
    total += weights.get("recency", 0) * recency_score
    total += weights.get("confidence", 0) * confidence
